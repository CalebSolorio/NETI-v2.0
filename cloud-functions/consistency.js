...
/**
 *  Assists with manipulating consistency in the database.
 *
...
function update(req, res) {
  if(req.body.name.replace(/\s/g, "").length > 0) {
    ...
    res.status(400).send('The name of the consistency must be provided.');
  }
}
          ...
          for (var appearance in drug.appearances) {
            if (appearances.hasOwnProperty(appearance)) {
              if(appearance.consistency_id == id) {
                res.status(403)
                  .send("Drugs are still dependent on this consistency.");
              }
            }
          }
        } else {
